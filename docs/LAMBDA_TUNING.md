...
> `run_assignment(split="eval", lambda_weights={wait:0.5, overload:0.1, balance:0.1, zone_mismatch:0.1})`

| 指标 | calib (140) | **eval (60)** | 结论 |
|------|:---:|:---:|------|
| 分配率 | 7.14% | **11.67%** | ✅ 无过拟合 |
| 科室匹配 | 0.80 | **0.86** | ✅ |
| 超负荷惩罚 | 68 | **35** | ✅ |
| 隔离床利用率 | 1.0 | **1.0** | 瓶颈二次证实 |
| 高危等待 | 130/140 | 53/60 | ~88% 一致 |

**结论**：推荐 λ 在独立 eval 集泛化良好（指标略优于 calib）；「隔离床不足」为系统级瓶颈，非 λ 可解。
