# Dump 交付说明（队友 restore）

> 更新：2026-08-02 · **dump 不入 GitHub**

## 人读摘要

| 文件 | schemas_only | 可用于 |
|------|--------------|--------|
| `icu_scheduling_P0-full_mimic_94458stays_20260802.dump` | **false** | ✅ SOFA + staging + CP-SAT / 滚动仿真 |
| `…20260727…`（旧） | false | 可用对照；优先用 20260802 |
| `…20260708…` | true / 过时 | ❌ 勿用 |

**SHA-256**：`cb72a741c0f3d092a8e4ed661a16dd64c0787833725abfdd59ba1c036c2c8294`

| 需求 | dump 是否够 |
|------|-------------|
| CP-SAT / plan / Streamlit | ✅ |
| 用已写入的 `feat.sofa_timeseries` | ✅（≈94,458） |
| 从零重算 SOFA（需 Layer0 labevents） | 需本机 `mimic` labs |
| online PPO 轨迹 | ❌ → `docs/PPO_SMOKE.md` |

```powershell
.\scripts\restore_layer1.ps1 -DumpFile .\dumps\icu_scheduling_P0-full_mimic_94458stays_20260802.dump
$env:PYTHONPATH = (Get-Location)
.\.venv\Scripts\python.exe -m application.simulate
```
